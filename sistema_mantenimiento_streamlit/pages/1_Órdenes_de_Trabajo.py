...
with st.form("nueva_orden"):
    ...
    if submitted:
        if descripcion and equipo_seleccionado:
            # Generar código
...
with col3:
    equipos_filtro = ["Todos"] + [eq.nombre for eq in session.query(Equipo).all()]
    filtro_equipo = st.selectbox("Equipo", equipos_filtro)
